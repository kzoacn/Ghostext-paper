...
\subsection{Detector and Watermark Context}
...
Given this landscape, Ghostext claims novelty at the systems-integration level: combining cryptographic payload normalization, authenticated packet framing, runtime configuration binding, retokenization-aware candidate control, and explicit failure observability in one deterministic pipeline. This revision now includes an initial same-backend NLS-style comparison slice, but it remains intentionally narrow and implementation-aligned rather than a historical reproduction of NLS/OD-Stega under their original GPT-2/LLAMA settings. We therefore do not make broad superiority claims over those baselines here, and whether this systems combination yields significant detector-facing gains remains an open empirical question for the remaining detector-focused stages.
