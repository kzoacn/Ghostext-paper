...
For the real-backend baseline, the released artifacts already use CJK prompts and CJK secret messages in the zh condition. Table~\ref{tab:examples} keeps the toy CJK row Unicode-escaped in the anonymous draft to avoid font/rendering dependence across review environments; a later public version should show a rendered CJK example directly. The current JSONL release logs metrics and metadata but not full cover strings; adding full-text release is tracked as a post-pause artifact task.
