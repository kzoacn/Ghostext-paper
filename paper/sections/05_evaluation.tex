...
\subsection{Setup}
We run two backends. The toy backend is used as a deterministic protocol-regression baseline. The real backend uses \texttt{llama.cpp} with a local Qwen3.5-2B GGUF model. All reported numbers come from rerunnable scripts that emit structured artifacts in \texttt{results/}.

\subsection{Results}
On the toy backend, matched-configuration round-trip succeeds on 4/4 cases (100\%). Fail-closed scenarios for wrong seed, wrong prompt, wrong passphrase, and mutated packet text all fail as expected (4/4, 100\%). In this run, mean embedding efficiency is 0.572 bits/token, and mean encode throughput is 556.75 tokens/s.
...
\midrule
zh\_short & Y & 118 & 1394 & 0.677 \\
en\_short & Y & 122 & 2525 & 0.387 \\
zh\_medium & Y & 157 & 1699 & 0.739 \\
en\_medium & Y & 154 & 3207 & 0.384 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Artifacts.}
Primary outputs are \texttt{results/minimal\_eval\_results.json}, \texttt{results/real\_backend\_eval\_results.json}, and \texttt{results/consolidated\_eval\_tables.md}.
