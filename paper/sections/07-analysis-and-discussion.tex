...
Configuration note: the released real-backend baseline uses $F_{\mathrm{tot}}=4096$ and $h_{\min}=0.0$, while implementation defaults are $F_{\mathrm{tot}}=65536$ and $h_{\min}=1.0$. All numeric examples below are labeled explicitly.
...
&=\sum_v \widetilde{P}_t(v)\log\!\left(1-\frac{\delta_t(v)}{Q_t(v)}\right).
\end{align}
For $u<1$, $-\log(1-u)\le \frac{u^2}{1-u}$, so with $u=\delta_t(v)/Q_t(v)$ and $Q_t(v)-\delta_t(v)=\widetilde{P}_t(v)\ge0$ we get
\begin{align}
D_{\mathrm{KL}}(\widetilde{P}_t\|Q_t)
...
This bound is loose but explicit. Under implementation defaults ($N_t\le16$, $F_{\mathrm{tot}}=65536$), it is at most $2.44\times10^{-4}$ nats for quantization alone. Under the released baseline setting ($N_t\le64$, $F_{\mathrm{tot}}=4096$), the corresponding worst-case ceiling is $1.56\times10^{-2}$ nats. Truncation and stability filtering can dominate this term in practice, so all terms should be logged together.
...
In this revision, $\widehat{B}_{\mathrm{TV}}$ is a defined diagnostic and the full real-backend measurement block remains pending (Stage 3).
...
The 64-bit width is therefore a deployment tradeoff in the current artifact, not a claim that 64 bits is universally optimal.
...
Claims about broad detector resistance and cross-family generalization remain pending dedicated detector and cross-family experiments. Active text modification in transit remains out of scope in this revision.
