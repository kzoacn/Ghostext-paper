...
&=\sum_{v:\widetilde{P}_t(v)>0} \widetilde{P}_t(v)\log\!\left(1-\frac{\delta_t(v)}{Q_t(v)}\right).
\end{align}
...
\begin{align}
D_{\mathrm{KL}}(\widetilde{P}_t\|Q_t)
&\le \sum_{v:\widetilde{P}_t(v)>0} \frac{\delta_t(v)^2}{Q_t(v)}.
\end{align}
...
The resulting bound is loose but explicit. Under implementation defaults ($N_t\le16$, $F_{\mathrm{tot}}=65536$), it is at most $2.44\times10^{-4}$ nats for quantization alone. Under the released baseline setting ($N_t\le64$, $F_{\mathrm{tot}}=4096$), the corresponding worst-case ceiling is $1.56\times10^{-2}$ nats. Truncation and stability filtering can dominate this term in practice, so all terms should be logged together.
