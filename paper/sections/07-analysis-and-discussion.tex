...
and $\beta_t$ is additional mass removed by tokenization-stability filtering.
...
Even when text-only quality is strong, these mixture shifts may be detectable by a traffic observer. Post-pause evaluation should therefore report retry-conditioned and unconditional length/latency distributions.
...
\begin{table}[t]
\caption{Per-step and per-message diagnostics to log for analysis and ablation.}
\label{tab:analysis-logs}
\begin{tabular}{p{0.21\linewidth}p{0.73\linewidth}}
\toprule
Quantity & Role in analysis \\
\midrule
...
Claims about broad detector resistance, active-edit robustness, and cross-family generalization remain pending post-pause experiments.
